...
public class Heap<T extends Comparable<T>> { // generics included to jump start you
	...
	public Heap( Collection<Comparable<T>> elements ) {
		...
		heap = (T[]) new Comparable[elements.size()];
		ArrayList<Comparable<T>> tempList = new ArrayList<Comparable<T>>(elements); //deep copy the collection
		tempList.toArray(heap);
	...
	private void heapDown( int index, int end ) {
		...
		int largest;
		if (leftInd <= heap.length - 1 && heap[leftInd].compareTo((T) heap[index]) > 0) {
			largest = leftInd;
		} else {
			largest = index;
		}
		
		if (rightInd <= heap.length - 1 && heap[rightInd].compareTo((T) heap[largest]) > 0) {
			largest = rightInd;
		}
	...
	private void swap(int index, int leftChildIndex) {
		Comparable<T> tempOne = heap[index];
		Comparable<T> tempTwo = heap[leftChildIndex];
		heap[index] = tempTwo;
		heap[leftChildIndex] = tempOne;
	}
	/**
	...
	public static <T extends Comparable<T> > List<T> sort( List<T> lst ) {
		Collection<Comparable<T>> temp = new ArrayList<Comparable<T>>(lst);
		Heap<T> tempHeap = new Heap<T>(temp);
		tempHeap.sort();
		List<T> sortedList = new ArrayList<T>();
		for (Comparable<T> item: tempHeap.heap) {
